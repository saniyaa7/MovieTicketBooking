...
class Ticket < ApplicationRecord
  ...
  message: "%{value} is not a valid seat_type" }, allow_blank: true, if: -> { seat_type.present? && seat_type.is_a?(Array) }
  before_validation :normalize

  def calculate_and_save_price(movie_show)
    total_price = 0
    seat_no = []
    if self.seat_book != self.seat_type.length
    ...
    if movie_show.seat_count == 0
      errors.add(:base, "All seats are full")
      return false
    end
    if movie_show.seat_count < self.seat_type.length
    ...
    self.transaction_id= "#{self.user_id}_#{self.movie_show_id}_#{Time.now.strftime('%Y%m%d%H%M%S')}"
    movie_show.save
  end
