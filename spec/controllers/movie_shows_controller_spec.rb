...
RSpec.describe Api::V1::MovieShowsController, type: :controller do
  ...
  describe 'POST #create' do
    context 'with valid parameters' do
      let(:valid_params) do
        {
          movie_show: attributes_for(:movie_show, language: 'English', seat_count: 100, show_start_time: DateTime.now,
                                                  show_end_time: DateTime.now + 2.hours, screen_no: 1, movie_id: create(:movie).id)
        }
      end
    ...
    context 'with invalid parameters' do
      let(:invalid_params) do
        {
          movie_show: { language: nil, seat_count: nil, show_start_time: DateTime.now,
                        show_end_time: DateTime.now + 2.hours, screen_no: 1, movie_id: create(:movie).id }
        }
      end
  ...
  describe 'DELETE #destroy' do
    ...
    it 'destroys the movie show' do
      ...
      end.to change(MovieShow, :count).by(-1)
